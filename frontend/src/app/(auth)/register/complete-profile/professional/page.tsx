// File: src/app/register/complete-profile/professional/page.tsx
'use client';
...
export default function CompleteProfessionalProfilePage() {
    ...
    // preenche name/email vindos do OAuth
    ...
    const steps = ['Dados Pessoais', 'Atuação', 'NR-1']
            ...
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_API_URL}/auth/complete-profile`,
                {
                    ...
                    body: JSON.stringify({
                        ...
                        cpf: '',               // opcional no DTO
                        role: 'profissional',  // atualiza para profissional
                    }),
                }
            )
        ...
        <div className="min-h-screen flex items-center justify-center bg-[#F8F4FF] p-4">
            <div className="w-full max-w-md bg-white p-6 rounded-lg shadow">
                <h2 className="text-2xl font-bold text-center mb-4">
                    Complete seu perfil de Profissional
                </h2>
                ...
                >
                    ...
                    {step === 2 && (
                        <>
                            <Label>Cargo</Label>
                            <Input
                                placeholder="Ex.: Psicólogo, Consultor..."
                                value={form.jobTitle}
                                onChange={e => setForm(f => ({ ...f, jobTitle: e.target.value }))}
                            />
                            <Label className="mt-4">Descrição Profissional</Label>
                            <Input
                                placeholder="Ex.: Especialista em saúde ocupacional..."
                                value={form.descricaoProfissional}
                                onChange={e => setForm(f => ({
                                    ...f, descricaoProfissional: e.target.value
                                }))}
                            />
                        </>
                    )}
                    ...
                    {step === 3 && (
                        <>
                            <Label>Como está a adequação à NR-1?</Label>
                            <Select
